...
if [ "$MODE" == "up" ]; then
  ...
  infoln "If network is not up, starting nodes with CLI timeout of '${MAX_RETRY}' tries and CLI delay of '${CLI_DELAY}' seconds and using database 'couchdb' ${CRYPTO_MODE}"
elif [ "$MODE" == "down" ]; then
